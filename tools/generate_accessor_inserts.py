...
# ---------------------------------------------------------------------------
# Registry and row model generation (future refactor support)
# ---------------------------------------------------------------------------
...
def _run_check(
    *,
    ...
    """
    if _is_generated_inserts_outdated(generated_code):
        return 1
    if (
        registry_output
...
def _is_generated_inserts_outdated(generated_code: str) -> bool:
    """Check the generated inserts module for staleness.

    Returns
    -------
    bool
        True when the generated content differs from the file on disk.
    """
