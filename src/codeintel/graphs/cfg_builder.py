...
1. Loads function metadata from storage (via catalog)
2. Delegates pure CFG/DFG construction to graphs.compute.cfg and graphs.compute.dfg
...
# Import compute layer for pure CFG/DFG construction
...
from codeintel.graphs.compute.cfg import BasicBlock as Block  # noqa: E402
from codeintel.graphs.compute.cfg import CFGEdge as Edge  # noqa: E402

# Alias the compute layer builder as CFGBuilder for backward compatibility
...
class DFGBuilder:
    ...
    def __init__(self, goid: int, blocks: list[Block], edges: list[Edge]) -> None:
        ...
        self.blocks = blocks
        self.edges = edges
        # Convert Edge to CFGEdge for compute layer compatibility
        from codeintel.graphs.compute.cfg import CFGEdge  # noqa: PLC0415

        cfg_edges = [CFGEdge(src=e.src, dst=e.dst, kind=e.kind) for e in edges]
