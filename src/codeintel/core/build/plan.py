...
build plans. A plan groups targets by pipeline module, provides time
...
- **PlanStage**: A group of steps in the same pipeline module
...
- **Batching**: Groups targets that run in the same pipeline stage
...
if TYPE_CHECKING:
    from codeintel.core.build.resolver import ResolutionResult
    from codeintel.core.build.targets import TargetGraph
...
"""Canonical execution order for pipeline modules."""
...
class PlanStep:
    ...
    module
        Pipeline module (ingestion, graphs, or analytics).
    plugin
...
@dataclass(frozen=True)
class PlanStage:
    """A group of steps that execute together in one pipeline module.
    ...
    ----------
    module
        Pipeline module for this stage.
    steps
