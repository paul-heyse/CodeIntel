...
def finalize_table(
    ...
    *,
    ...
    if aligned is None:
        return _missing_contract_result(table, spec)
    ...
    errors = _concat_errors(error_tables, aligned)
...
def _empty_finalize_result(spec: FinalizeSpec) -> FinalizeResult:
    ...
    return FinalizeResult(
        good=empty,
        errors=_empty_error_table(empty, key_fields=()),
        alignment=_empty_alignment_table(),
        stats=_empty_stats_table(),
    )


def _missing_contract_result(table: pa.Table, spec: FinalizeSpec) -> FinalizeResult:
    errors = _error_table_for_missing_contract(table, spec.table_key)
    alignment = _empty_alignment_table()
...
def _error_table_for_missing_contract(table: pa.Table, table_key: str) -> pa.Table:
    if table.num_rows == 0:
        return _empty_error_table(table, key_fields=())
...
def _empty_error_table(table: pa.Table, key_fields: Sequence[str]) -> pa.Table:
    ...
    fields.extend([table.schema.field(name) for name in key_fields if name in table.column_names])
    return pa.Table.from_batches([], schema=pa.schema(fields))


def _concat_errors(errors: Sequence[pa.Table], table: pa.Table) -> pa.Table:
