...
def grouped_rollup_table(
    ...
    *,
    ...
    aggregates
        Aggregate specifications for the plan.
    order_by
        Optional ordering keys to apply after aggregation.
    ctx
    ...
    plan = plan.aggregate(
        keys=[E.field(name) for name in keys] if keys else None,
