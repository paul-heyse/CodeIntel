...
if TYPE_CHECKING:
    ...
    from codeintel.core.duckdb_types import DuckDBConnection, Expression
    from codeintel.core.schemas.contract_primitives import DatasetContract


class RelationGateway(Protocol):
    """Protocol for gateways exposing DuckDB relations by table key."""
...
def safe_table_exists(gateway: RelationGateway, table_key: str) -> bool:
...
def safe_get_columns(gateway: RelationGateway, table_key: str) -> set[str]:
...
def safe_count_orphan_refs(gateway: RelationGateway, fk: ForeignKeyRef) -> int:
