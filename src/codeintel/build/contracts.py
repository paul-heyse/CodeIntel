...
__all__ = [
    ...
    "PLACEHOLDER_SCHEMA_DESCRIPTION",
    "ArtifactSpec",
    "Column",
    "ColumnType",
    "Index",
    ...
    "placeholder_table_schema",
]

PLACEHOLDER_COLUMN_NAME = "__placeholder"
PLACEHOLDER_SCHEMA_DESCRIPTION = "Placeholder schema generated by OutputContract.simple"
...
class OutputContract:
    ...
    def simple(
        ...
        """
        tables = [placeholder_table_schema(key) for key in table_keys]
...
def placeholder_table_schema(table_key: str) -> TableSchema:
    """Return a minimal placeholder TableSchema for a table key.
    ...
    -------
    TableSchema
        Table schema with a placeholder column.

    Raises
    ------
    ValueError
        If table_key is missing a schema or name component.
    """
    if "." not in table_key:
        message = f"Table key must include schema and name: {table_key}"
        raise ValueError(message)
    schema, name = split_table_key(table_key)
    return TableSchema(
        schema=schema,
        name=name,
        columns=[Column(PLACEHOLDER_COLUMN_NAME, "VARCHAR", nullable=True)],
        description=PLACEHOLDER_SCHEMA_DESCRIPTION,
    )


def is_placeholder_schema(table_schema: TableSchema) -> bool:
    """Return True when a TableSchema is a placeholder sentinel.

    Returns
    -------
    bool
        True when the schema is a placeholder.
    """
    if len(table_schema.columns) != 1:
        return False
    column = table_schema.columns[0]
    return column.name == PLACEHOLDER_COLUMN_NAME and column.type == "VARCHAR"
