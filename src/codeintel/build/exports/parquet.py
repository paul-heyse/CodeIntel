...
def _export_dataset_parquet(
    ...
    *,
    ...
    """Export a single dataset to Parquet with incremental support.
    ...
    try:
        ...
        export_parquet_for_table(gateway, target.table_name, target.output_path)
...
def export_all_parquet(
    ...
    *,
    ...
    selected = select_dataset_tables(dataset_mapping, parquet_mapping, opts.datasets)
    ...
    for dataset_name, table_name in sorted(selected.items()):
        ...
        exported = _export_dataset_parquet(gateway, target, opts=opts)
