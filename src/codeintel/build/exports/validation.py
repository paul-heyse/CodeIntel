...
from codeintel.core.validation.schema_constraints import validate_parquet_path
...
def _get_generated_schema(table_key: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        log.debug("Schema lookup failed for %s: %s", table_key, exc)
        return None
...
def validate_export_files(
    ...
    *,
    ...
    schema = get_json_schema_for_table_schema(table_schema) or _get_generated_schema(table_key)
    if schema is None:
        label = dataset_name or table_key
