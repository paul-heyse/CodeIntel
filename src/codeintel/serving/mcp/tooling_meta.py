"""Tooling metadata helpers shared across MCP tools and resources.
...
def runtime_versions(*, tools: tuple[str, ...] = _DEFAULT_TOOL_NAMES) -> dict[str, str]:
    ...
    ----------
    tools
        Package names to query with ``importlib.metadata.version``.

    Returns
    -------
    dict[str, str]
        Mapping of package name to version (or "not-installed").
    """
...
def tooling_mismatch_warnings(
    ...
    *,
    ...
    ----------
    environment
        Snapshot environment dictionary (expected to contain a ``tools`` mapping).
    runtime
        Optional precomputed runtime versions; when None, computed via ``runtime_versions()``.

    Returns
    -------
    tuple[str, ...]
        Stable ordered warning strings.
    """
