...
class ResourceNotFoundError(ResourceError):
    ...
    def __init__(self, resource_type: type | str) -> None:
        ...
        if isinstance(resource_type, str):
            ...
            self.resource_type_name = resource_type.__name__


class ResourceRegistry:
    """Central registry for lazy resource access in ingestion.
    ...
    >>> provider = registry.get(ModuleProvider)
    >>> modules = provider.get()
    >>> # String-based lookup (avoids import cycles):
    >>> same_provider = registry.get_by_name("ModuleProvider")
    """

    def __init__(self) -> None:
        """Initialize an empty registry."""
        self._providers: dict[type[object], ResourceProvider[Any]] = {}
    ...
    def register(
        ...
        *,
        ...
        provider class (e.g., ModuleProvider) while the provider returns
        a different type (e.g., list[ModuleRecord]).
    ...
    def has(self, resource_type: type[object]) -> bool:
    ...
    def invalidate(self, resource_type: type[object] | None = None) -> None:
    ...
    @property
    def registered_types(self) -> frozenset[type[object]]:
