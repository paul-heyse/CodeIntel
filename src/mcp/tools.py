...
class McpServerConfig(BaseModel):
    ...
    def _expand_paths(cls, values):
        """
        Normalize repo_root/db_path to absolute paths early.
        """
    ...
    def _check_backend(cls, values):
        """
        Enforce that db_path/api_base_url are present for the appropriate mode.
        """
        mode: McpMode = values.get("mode", "local_db")
        db_path: Optional[Path] = values.get("db_path")
        api_base_url: Optional[str] = values.get("api_base_url")
