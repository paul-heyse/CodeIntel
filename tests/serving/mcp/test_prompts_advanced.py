...
@pytest.mark.anyio
async def test_mcp_list_prompts_includes_tags_and_version_meta(
...
@pytest.mark.anyio
async def test_mcp_get_prompt_wizard_query_view_uses_elicitation(
    serving_snapshot_factory: ServingSnapshotFactory,
) -> None:
    ...
    async with harness.mcp_client(
        settings_overrides={"hot_swap": False},
        ...
        invocation_texts = [text for text in texts if '"tool"' in text and '"arguments"' in text]
