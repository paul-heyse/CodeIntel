...
if TYPE_CHECKING:
    ...
    from tests._helpers.harnesses.serving_app import ServingSettingsOverrides
...
def _make_harness(tmp_path: Path) -> ServingAppHarness:
    snapshot = ServingSnapshotFactory(tmp_path).demo_snapshot()
...
async def test_mcp_semantic_export_returns_uri(tmp_path: Path) -> None:
    ...
    async with harness.mcp_client(settings_overrides=_default_mcp_settings()) as client:
...
async def test_mcp_static_resources_available(tmp_path: Path) -> None:
    ...
    harness = _make_harness(tmp_path)
    settings = _default_mcp_settings()
    async with harness.mcp_client(settings_overrides=settings) as client:
        # List all resources
...
async def test_mcp_resource_meta_environment(tmp_path: Path) -> None:
    ...
    harness = _make_harness(tmp_path)
    settings: ServingSettingsOverrides = {"hot_swap": False}
    async with harness.mcp_client(settings_overrides=settings) as client:
