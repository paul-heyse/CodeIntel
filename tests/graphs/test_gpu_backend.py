...
def test_enable_nx_cugraph_backend_missing_module(monkeypatch: pytest.MonkeyPatch) -> None:
    """Helper should raise RuntimeError when nx_cugraph is absent."""
    original_import = importlib.import_module

    def _fake_import(name: str, *_args: object, **_kwargs: object) -> object:
        if name == "nx_cugraph":
            raise ImportError
...
def test_enable_nx_cugraph_backend_invokes_setter(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Helper should call set_default_backend when present."""
    called = {"set": False}
    fake_module = types.SimpleNamespace(set_default_backend=lambda: called.update({"set": True}))
    original_import = importlib.import_module
    ...
    monkeypatch.setattr(importlib, "import_module", _fake_import)
    cfg = GraphBackendConfig(use_gpu=True, backend="nx-cugraph", strict=True)
    result = maybe_enable_nx_gpu(cfg)
    if not called["set"]:
