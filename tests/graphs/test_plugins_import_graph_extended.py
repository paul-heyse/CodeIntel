...
def _make_gateway() -> StorageGateway:
    """Create a gateway for import graph tests.

    Returns
    -------
    StorageGateway
        Configured gateway with all schemas applied.
    """
...
def test_build_import_graph_empty_catalog(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert _count_import_modules(gateway, REPO, COMMIT) == EXPECTED_ZERO
        assert _count_import_edges(gateway, REPO, COMMIT) == EXPECTED_ZERO
    finally:
        gateway.close()


def test_build_import_graph_single_module_no_imports(tmp_path: Path) -> None:
...
''',
        ...
        assert _count_import_modules(gateway, REPO, COMMIT) == EXPECTED_ONE
        assert _count_import_edges(gateway, REPO, COMMIT) == EXPECTED_ZERO
    finally:
        gateway.close()


def test_build_import_graph_simple_chain(tmp_path: Path) -> None:
...
''',
        ...
        module_count = _count_import_modules(gateway, REPO, COMMIT)
        assert module_count >= EXPECTED_THREE

        # Verify layer ordering: A (0) -> B (1) -> C (2)
        layers = _get_module_layers(gateway, REPO, COMMIT)
...
def test_build_import_graph_cycle_detection(tmp_path: Path) -> None:
...
''',
        ...
        scc_ids = _get_module_scc_ids(gateway, REPO, COMMIT)
...
def test_build_import_graph_diamond_pattern(tmp_path: Path) -> None:
...
def test_build_import_graph_mixed_imports(tmp_path: Path) -> None:
...
''',
        ...
        module_count = _count_import_modules(gateway, REPO, COMMIT)
        edge_count = _count_import_edges(gateway, REPO, COMMIT)
...
def test_build_import_graph_missing_file_skipped(tmp_path: Path) -> None:
...
def test_build_import_graph_parse_error_skipped(tmp_path: Path) -> None:
...
def test_build_import_graph_persists_modules_table(tmp_path: Path) -> None:
...
def test_build_import_graph_persists_edges_table(tmp_path: Path) -> None:
