...
def _make_plugin(
    ...
    *,
    ...
    """Create a CstExtractPlugin with recording adapters.
...
@pytest.mark.anyio
async def test_step_errors_logged_as_warnings(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """Step errors should be logged as warnings but still return success.

    The plugin handles errors gracefully by logging them without failing.
    """
    repo_root = build_repo_tree(tmp_path / "repo", {"pkg/mod.py": "x = 1\n"})
    captured = StepCallCapture()
    plugin = _make_plugin(
    ...
    # Plugin returns success but logs errors as warnings
    expect_true(result.success is True)
    expect_in("boom", caplog.text)
