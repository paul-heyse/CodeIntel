...
async def test_execute_passes_modules_and_returns_counts(
    ...
    expect_equal(result.row_counts, {"core.docstrings": 5})
...
async def test_execute_uses_gateway_when_modules_missing(
    ...
    module_record = captured.modules[0]
    expect_equal(module_record.rel_path, "pkg/doc_db.py")
    expect_equal(module_record.file_path, repo_root / "pkg/doc_db.py")
    expect_equal(captured.repo, DEFAULT_REPO)
    expect_equal(captured.commit, DEFAULT_COMMIT)
