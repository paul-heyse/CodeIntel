"""Shared MCP backend fixtures for tests (non-conftest plugin)."""
...
def _build_components(
    ...
    *,
    ...
    -------
    McpBackendComponents
        Aggregated gateway, query, service, observability, and backend components.
    """
...
def mcp_backend_factory() -> Callable[..., McpBackendComponents]:
    ...
    Callable[..., McpBackendComponents]
        Factory that constructs backend components for provided gateway and repo info.
    """
...
def mcp_backend_components(
    ...
    -------
    McpBackendComponents
        Backend bundle derived from the provisioned repository snapshot.
    """
...
def mcp_backend(mcp_backend_components: McpBackendComponents) -> McpBackendComponents:
    ...
    -------
    McpBackendComponents
        Backend bundle for the provisioned gateway snapshot.
    """
...
def mcp_query_service(mcp_backend_components: McpBackendComponents) -> DuckDBQueryService:
    ...
    -------
    DuckDBQueryService
        Query service instance wired to the provisioned gateway snapshot.
    """
