...
def test_data_model_field_schema_validates_fixture(tmp_path: Path) -> None:
    ...
    path.write_text(
        (
            ...
            '"extras":{"constraints":{}},"source":"pydantic_field","rel_path":"pkg/mod.py",'
            '"lineno":10,"created_at":"2025-01-01T00:00:00Z"}'
        ),
        encoding="utf-8",
    )
    exit_code = validate_export_files("analytics.data_model_fields", [path])
    if exit_code != 0:
...
def test_data_model_relationship_schema_validates_fixture(tmp_path: Path) -> None:
    ...
    path.write_text(
        (
            ...
            '"extras":{"evidence":{"source":"hint"}},"rel_path":"pkg/mod.py","lineno":20,'
            '"created_at":"2025-01-01T00:00:00Z"}'
        ),
        encoding="utf-8",
    )
    exit_code = validate_export_files("analytics.data_model_relationships", [path])
    if exit_code != 0:
