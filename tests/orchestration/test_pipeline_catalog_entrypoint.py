...
def test_pipeline_steps_use_function_catalog(
    pipeline_env: PipelineEnv, pipeline_coverage_artifact: Path
) -> None:
    """Ensure pipeline steps and builders all consume the shared function catalog."""
    build_graph_and_symbols(pipeline_env)

    def _load_coverage(_cfg: TestCoverageStepConfig) -> Coverage:
        return load_coverage(pipeline_coverage_artifact, _cfg)

    compute_test_coverage_edges(
        pipeline_env.gateway,
        pipeline_env.ctx.config_builder().test_coverage(
            coverage_file=pipeline_coverage_artifact,
    ...
    catalog = load_function_catalog(pipeline_env.gateway, repo=REPO, commit=COMMIT)
    callee_goid = catalog.lookup_goid("pkg/a.py", 1, 2, "pkg.a.callee")
    caller_goid = catalog.lookup_goid(
        "pkg/b.py",
        pipeline_env.caller_lines[0],
        pipeline_env.caller_lines[1],
        "pkg.b.caller",
    )
    ...
    _assert(
        ...
        in {
            row[0]
            for row in pipeline_env.gateway.con.execute(
