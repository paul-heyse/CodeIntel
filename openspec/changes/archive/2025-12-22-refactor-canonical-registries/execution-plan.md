# Refactor Canonical Registries - Execution Plan

## Status: Completed
All migration batches in this plan were executed. Target overrides now live in Hamilton
metadata, table_registry DAG outputs were removed, declared_schemas is source-only, and
canonical catalogs are the sole discovery surface. This plan is retained for historical
traceability of the executed sequence.

This plan enumerates the remaining scope and provides an ordered,
target-by-target migration list with the exact schema snippets to move from
`src/codeintel/core/schemas/table_registry.py` into each target's
`TargetSpecOptions(override_tables=...)`.

## Preconditions
- Work on a clean slice of the remaining scope (no parallel refactors).
- Keep Hamilton as the single source of truth; only non-inferable outputs use overrides.
- Remove DAG outputs from declared schemas after each batch.

## Inferable Table Keys (computed)
Hamilton native inference can supply schemas for the following output tables:
- `analytics.coverage_functions`
- `analytics.goid_risk_factors`

## Table Registry Impact
These keys are currently in `TABLE_SCHEMAS` and are produced by Hamilton targets.
They should be removed from `table_registry.py` once overrides are in place.
...
Keys in `TABLE_SCHEMAS` not produced by Hamilton targets (retain):
...
## Step-by-step Migration (Ordered)
### Batch 1 - Core/Ingestion outputs

#### Target `ast`
- File: `src/codeintel/build/hamilton/native/ingestion/extraction_targets.py`
- Table keys: `core.ast_nodes`, `core.ast_metrics`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `modules`
- File: `src/codeintel/build/hamilton/native/ingestion/ingest_targets.py`
- Table keys: `core.modules`, `core.file_state`, `core.repo_map`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- Table keys: `core.goids`, `core.goid_crosswalk`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- Table keys: `core.scip_symbols`, `core.scip_occurrences`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `typing`
- File: `src/codeintel/build/hamilton/native/ingestion/ingest_targets.py`
- Table keys: `analytics.typedness`, `analytics.static_diagnostics`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/ingestion/ingest_targets.py`
- Table keys: `analytics.config_values`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `coverage_ingest`
- File: `src/codeintel/build/hamilton/native/ingestion/ingest_targets.py`
- Table keys: `analytics.coverage_lines`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `tests_ingest`
- File: `src/codeintel/build/hamilton/native/ingestion/ingest_targets.py`
- Table keys: `analytics.test_catalog`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
### Batch 2 - Graph outputs

#### Target `call_graph`
- File: `src/codeintel/build/hamilton/native/graphs/call_graph.py`
- Table keys: `graph.call_graph_nodes`, `graph.call_graph_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `cfg`
- File: `src/codeintel/build/hamilton/native/graphs/cfg_dfg.py`
- Table keys: `graph.cfg_blocks`, `graph.cfg_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `import_graph`
- File: `src/codeintel/build/hamilton/native/graphs/import_graph.py`
- Table keys: `graph.import_modules`, `graph.import_graph_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `symbol_uses`
- File: `src/codeintel/build/hamilton/native/graphs/graph_targets.py`
- Table keys: `graph.symbol_use_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/graphs/graph_targets.py`
- Table keys: `graph.v_function_call_counts`, `graph.v_call_depth_stats`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
### Batch 3 - Analytics outputs (dependency/config + coverage)

#### Target `entrypoints`
- File: `src/codeintel/build/hamilton/native/analytics/dependency_targets.py`
- Table keys: `analytics.entrypoints`, `analytics.entrypoint_tests`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/dependency_targets.py`
- Table keys: `analytics.external_dependencies`, `analytics.external_dependency_calls`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/config_graph_targets.py`
- Table keys: `analytics.config_data_flow`, `analytics.config_graph_metrics_keys`, `analytics.config_graph_metrics_modules`, `analytics.config_projection_key_edges`, `analytics.config_projection_module_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/config_graph_targets.py`
- Table keys: `analytics.cfg_function_metrics`, `analytics.cfg_block_metrics`, `analytics.cfg_function_metrics_ext`, `analytics.dfg_function_metrics`, `analytics.dfg_block_metrics`, `analytics.dfg_function_metrics_ext`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/coverage_targets.py`
- Table keys: `analytics.behavioral_coverage`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `coverage_test_edges`
- File: `src/codeintel/build/hamilton/native/analytics/coverage_targets.py`
- Table keys: `analytics.test_coverage_edges`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/coverage_targets.py`
- Table keys: `analytics.coverage_functions`
- Inference-backed keys (do not add overrides; remove from registry):
  - `analytics.coverage_functions`

### Batch 4 - Analytics outputs (function + metadata + profiles)

#### Target `function_metrics`
- File: `src/codeintel/build/hamilton/native/analytics/function_metrics.py`
- Table keys: `analytics.function_metrics`, `analytics.function_types`, `analytics.function_validation`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `risk_factors`
- File: `src/codeintel/build/hamilton/native/analytics/risk_factors.py`
- Table keys: `analytics.goid_risk_factors`
- Inference-backed keys (do not add overrides; remove from registry):
  - `analytics.goid_risk_factors`

#### Target `function_effects`
- File: `src/codeintel/build/hamilton/native/analytics/function_detail_targets.py`
- Table keys: `analytics.function_effects`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/function_detail_targets.py`
- Table keys: `analytics.function_contracts`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `data_models`
- File: `src/codeintel/build/hamilton/native/analytics/metadata_targets.py`
- Table keys: `analytics.data_models`, `analytics.data_model_fields`, `analytics.data_model_relationships`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metadata_targets.py`
- Table keys: `analytics.data_model_usage`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metadata_targets.py`
- Table keys: `analytics.function_ast_features`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `profiles`
- File: `src/codeintel/build/hamilton/native/analytics/metadata_targets.py`
- Table keys: `analytics.function_profile`, `analytics.file_profile`, `analytics.module_profile`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `hotspots`
- File: `src/codeintel/build/hamilton/native/analytics/hotspots.py`
- Table keys: `analytics.hotspots`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
### Batch 5 - Analytics outputs (metrics, subsystems, tests)

#### Target `function_history`
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.function_history`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.history_timeseries`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/graphs/graph_targets.py`
- Table keys: `analytics.graph_metrics_functions`, `analytics.graph_metrics_functions_ext`, `analytics.graph_metrics_modules`, `analytics.graph_metrics_modules_ext`, `analytics.graph_stats`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/graphs/graph_targets.py`
- Table keys: `analytics.graph_validation`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.symbol_graph_metrics_modules`, `analytics.symbol_graph_metrics_functions`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.subsystem_graph_metrics`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.subsystem_agreement`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `subsystems`
- File: `src/codeintel/build/hamilton/native/analytics/subsystem_targets.py`
- Table keys: `analytics.subsystems`, `analytics.subsystem_modules`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/classification_targets.py`
- Table keys: `analytics.semantic_roles_functions`, `analytics.semantic_roles_modules`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
- File: `src/codeintel/build/hamilton/native/analytics/metrics_targets.py`
- Table keys: `analytics.test_graph_metrics_tests`, `analytics.test_graph_metrics_functions`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
#### Target `test_profile`
- File: `src/codeintel/build/hamilton/native/analytics/classification_targets.py`
- Table keys: `analytics.test_profile`
- Move these schema snippets into `TargetSpecOptions(override_tables=...)`:
...
## Cleanup Steps (After All Batches)
1. Remove all migrated keys from `src/codeintel/core/schemas/table_registry.py`.
2. Tighten `src/codeintel/build/hamilton/native/target_spec_helpers.py`:
   - Remove fallback to `TABLE_SCHEMAS` for non-overridden outputs.
   - Remove placeholder schema usage once all non-inferable outputs have overrides.
3. Remove unused schema helpers if no longer referenced:
   - `placeholder_table_schema` / `is_placeholder_schema` in `src/codeintel/build/contracts.py`
   - `merge_with_derived_schemas` in `src/codeintel/core/schemas/table_registry.py`
4. Update OpenSpec docs and tasks to reflect completion:
   - `openspec/changes/refactor-canonical-registries/tasks.md`
   - `openspec/changes/refactor-canonical-registries/specs/build-execution/spec.md`
