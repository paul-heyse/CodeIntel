...
- [x] 1.2 Consolidate compute modules into DAG-first, pure transforms with Hamilton
      materializers as the only write path.
...
- [x] 1.9 Centralize hashing/time/serialization helpers in core modules and remove
      duplicate utility implementations.

## 2. Migration and Cleanup
- [x] 2.1 Remove legacy registries, duplicate serializers, and redundant helper modules.
- [x] 2.2 Update integration points (CLI/build/serving) to use the new services and
      execution context.
- [x] 2.3 Update Hamilton tags/metadata to align with the consolidated registry service.
